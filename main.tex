...
\edition   {1}{October 2025 (Version 1.4)}
...
The coin allocation in the Pactus blockchain is distributed as follows:

\begin{table}[H]
  \centering
  \caption{Coin Allocation for Pactus}
  \begin{tabular}{|l|>{\raggedright\arraybackslash}p{9.2cm}|l|c|}
    \hline
			\textbf{Title} & \textbf{Address} & \textbf{Coin Allocation} & \textbf{\%} \\
    \hline
    Treasury & \texttt{000000000000000000000000000000000000000000} & 33.6 Million coins & 80\% \\
    \hline
    VC Allocation & \href{https://pacviewer.com/address/pc1zprhnvcsy3pthekdcu28cw8muw4f432hkwgfasv}{\texttt{pc1zprhnvcsy3pthekdcu28cw8muw4f432hkwgfasv}} & 6.3 Million coins & 15\% \\
    \hline
    Community & \href{https://pacviewer.com/address/pc1zs64vdggjcshumjwzaskhfn0j9gfpkvche3kxd3}{\texttt{pc1zs64vdggjcshumjwzaskhfn0j9gfpkvche3kxd3}} & 2.1 Million coins & 5\% \\
    \hline
  \end{tabular}
\end{table}

{\footnotesize \textit{Note:} This chart reflects the current allocation after the activation of \href{https://pips.pactus.org/PIPs/pip-43}{PIP-43}.}

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.5]{coins.png}
	\caption{Pactus coin allocation }
	\label{fig:coins}
	\centering
\end{figure}
...
\begin{itemize}
  \item
    Treasury Account: The Treasury account is a special account in the Pactus blockchain that holds $33.6$ million coins at the genesis time. The treasury address is defined as: 000000000000000000000000000000000000000000. The address type is 0, and therefore, it doesn’t have any key pair associated with it. Every time a block is created, one coin from the Treasury account transfers to the proposer account as a block reward.
  \item
...
\subsection{Block Rewards} The Treasury mints one PAC every 10 seconds and distributes it as the block reward. This predictable, rate-limited issuance increases circulating supply by 8,640 PAC per day (\(\approx 3.15\) million PAC per year). Under PIP‑43, each block reward is split 70\% to the proposing validator and 30\% to the Pactus Foundation, aligning validator incentives with long-term ecosystem funding.

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.5]{pactus-reward-distribution.png}
	\caption{Pactus reward distribution }
	\label{fig:pactus-reward-distribution}
	\centering
\end{figure}

\noindent The reward split mechanism, introduced in \href{https://pips.pactus.org/PIPs/pip-43}{PIP-43}, modifies how block rewards are distributed in Pactus:
\begin{itemize}
  \item \textbf{70\% to Validators:} The validator who proposes a block receives 0.7 coins as their reward.
  \item \textbf{30\% to Pactus Foundation:} The remaining 0.3 coins are distributed to the Pactus Foundation addresses.
\end{itemize}
This mechanism enhances sustainability and trust in the Pactus economy by providing predictable, transparent funding for the foundation's operations and ecosystem growth.
